<?php
/**
 * Import each route's staged hero content.
 *
 ...
 *
 * Run: wp eval-file gr-import-heroextra.php <sections.json> --path=.
 */
...
if (!$file || !file_exists($file)) {
    WP_CLI::error('Usage: wp eval-file gr-import-heroextra.php <sections.json>');
}
...
foreach ($sections as $route => $html) {
    ...
    update_post_meta($id, '_gr_hero_extra_html', $html);
    $bytes += strlen($html);
    $applied++;
}

WP_CLI::success(sprintf('Imported hero extras for %d routes (%s).', $applied, size_format($bytes)));
if ($missing) {
